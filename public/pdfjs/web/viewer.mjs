...
class PDFPrintService {
  constructor({
    ...
    this.water_mark_text = AppOptions.get("water_mark_text");
    this.pdfDocument = pdfDocument;
  ...
  useRenderedPage() {
    ...
    if (this.water_mark_img_obj) {
      ...
      ctx.fillRect(0, 0, scratchCanvas.width, scratchCanvas.height);
      ctx.fill();
    } else if (this.water_mark_text) {
      const waterMarkCanvas = document.createElement("canvas");
      const ctx = scratchCanvas.getContext("2d");
      const textConfig = {
        text: this.water_mark_text,
      ...
      tempCtx.font = textConfig.font;
      const textWidth = tempCtx.measureText(textConfig.text).width;
      const textHeight = parseInt(textConfig.font);
      waterMarkCanvas.width = textWidth + textConfig.spacing;
      waterMarkCanvas.height = textHeight + textConfig.spacing;
      const waterMarkCtx = waterMarkCanvas.getContext("2d");
      waterMarkCtx.font = textConfig.font;
      waterMarkCtx.fillStyle = textConfig.color;
      waterMarkCtx.translate(waterMarkCanvas.width / 2, waterMarkCanvas.height / 2);
      waterMarkCtx.rotate(textConfig.rotate * Math.PI / 180);
      waterMarkCtx.fillText(textConfig.text, -textWidth / 2, textHeight / 2);
      const pattern = ctx.createPattern(waterMarkCanvas, "repeat");
      ctx.fillStyle = pattern;
      ctx.fillRect(0, 0, scratchCanvas.width, scratchCanvas.height);
      ctx.fill();
...
class PDFThumbnailView {
  constructor({
    ...
    this.water_mark_text = water_mark_text;
    this.id = id;
  ...
  createTextImage(text, options = {}) {
    ...
    const diagonal = Math.ceil(Math.sqrt(textWidth ** 2 + textHeight ** 2)) + padding * 2;
  ...
  async #convertCanvasToImage(canvas) {
    ...
    if (this.water_mark_img_obj || this.water_mark_text) {
      let imgElement = "";
      if (this.water_mark_img_obj) {
        imgElement = this.water_mark_img_obj;
      } else if (this.water_mark_text) {
        imgElement = await this.createTextImage(this.water_mark_text);
      }
      const waterMarkCanvas = document.createElement("canvas");
      waterMarkCanvas.width = imgElement.width / 10;
      waterMarkCanvas.height = imgElement.height / 10;
      const waterMarkCtx = waterMarkCanvas.getContext("2d");
      waterMarkCtx.drawImage(imgElement, 0, 0, waterMarkCanvas.width, waterMarkCanvas.height);
    ...
    firstPagePromise.then(firstPdfPage => {
      ...
      for (let pageNum = 1; pageNum <= pagesCount; ++pageNum) {
        const thumbnail = new PDFThumbnailView({
          ...
          water_mark_text: AppOptions.get("water_mark_text")
        });
...
class Watermark {
  ...
  loadImageWithScale(dataUrl, scale = 1) {
  ...
  async _wrapDiv(dataUrl) {
    ...
    const {
      width,
      height
    } = await this.loadImageWithScale(dataUrl, scale);
    const div = document.createElement("div");
    div.id = id;
...
class PDFPageView extends BasePDFPageView {
  ...
  constructor(options) {
    ...
    this.water_mark_text = options.water_mark_text;
  ...
  async draw() {
    ...
    console.log("pdfPage", this.water_mark_text);
    if (!pdfPage) {
      this.renderingState = RenderingStates.FINISHED;
      throw new Error("pdfPage is not loaded");
    }
...
const PDFViewerApplication = {
  ...
  async initialize(appConfig) {
    ...
    } else if (watermarkData.has("water_mark_text")) {
      const water_mark_text = watermarkData.get("water_mark_text");
      AppOptions.set("water_mark_text", water_mark_text);
    }
    try {
